...
# ==============================================================================
# 4. POP-UP (AKTIENNAME & ANALSYSEN INTEGRATION mit Volumen-Check)
# ==============================================================================
...
def show_details_popup(ticker, company_name):
    with st.spinner("Lade optimierte Daten aus dem Cache..."):
        try:
            ...
            if df_base.empty:
                st.error("Keine Daten von Yahoo Finance erhalten.")
                return
            ...
            # Volumens-Verhältnis berechnen (Aktuelles Volumen vs. 20-Tage SMA-Volumen)
            df_base['Vol_SMA20'] = df_base['Volume'].rolling(window=20).mean()
            vol_ratio = df_base['Volume'].iloc[-1] / df_base['Vol_SMA20'].iloc[-1] if df_base['Vol_SMA20'].iloc[-1] > 0 else 1.0
            ...
            with meta_col3:
                st.metric("Volumen-Faktor (20d)", f"{vol_ratio:.2f}x")
            with meta_col4:
            ...
            # Dynamische Signal-Ampel Logik
            if rsi_aktuell > 70:
                ampel_signal = "🔴 VERKAUFEN (Sell)"
                grund = "Der RSI ist überkauft (> 70). Das Korrekturrisiko ist kurzfristig erhöht."
            elif rsi_aktuell < 30:
                if vol_ratio >= 1.5:
                    ampel_signal = "💥 ULTRA-BUY (RSI < 30 + Volumenausbruch)"
                    grund = f"Der RSI ist mit {rsi_aktuell:.2f} stark überverkauft UND das Volumen ist {vol_ratio:.2f}-mal so hoch wie im Schnitt! Mögliche Kapitulation/Bodenbildung."
                else:
                    ampel_signal = "🟢 KAUFEN (Buy)"
                    grund = "Der RSI ist überverkauft (< 30). Die Aktie ist technisch bereit für eine technische Gegenbewegung."
            ...
            def render_chart(df_chart, title_suffix):
                if df_chart.empty or len(df_chart) < 2:
                    st.warning(f"Keine ausreichenden Intraday-Daten im Cache.")
                    return
                ...
                # ROW 3: MACD & VOLUMEN ALS HISTOGRAMM-BACKGROUND
                ...
                fig.add_trace(gr.Bar(x=df_chart.index, y=df_chart['MACD_Hist'], name='MACD Hist', marker_color='lightgray', opacity=0.7), row=3, col=1)
            ...
            with tab3:
                ...
                df_4h = df_1h.resample('4h').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last', 'Volume': 'sum'}).dropna() if not df_1h.empty else pd.DataFrame()
                render_chart(df_4h, "4 Std")
            with tab4:
                render_chart(df_base.tail(30), "1 Tag")
                
        except Exception as e:
            st.error(f"Fehler im Analyse-Fenster: {e}")

# ==============================================================================
# 5. SEITENLEISTE (Inklusive Volumen-Trigger)
# ==============================================================================
...
rsi_min = st.sidebar.slider("Minimaler RSI-Wert", 0, 100, 0, step=1)
rsi_max = st.sidebar.slider("Maximaler RSI-Wert", 0, 100, 30, step=1) # Standardmäßig auf 30 gesetzt für Unter-30-Scans

st.sidebar.write("---")
st.sidebar.header("🔥 Volumen-Trigger")
volumen_ausbruch = st.sidebar.toggle("Nur mit Volumenausbruch (RSI < 30)", value=True)
vol_threshold = st.sidebar.slider("Mindest-Faktor vs. 20-Tage Schnitt", 1.0, 3.0, 1.5, step=0.1, help="Beispiel: 1.5 bedeutet, das heutige Volumen muss mindestens 150% des 20-Tage-Durchschnitts betragen.")
...
search_query = st.sidebar.text_input("Aktienname oder Ticker eingeben:")
if search_query:
