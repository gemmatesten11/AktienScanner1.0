...
# Streamlit Page Config für ein breiteres Layout (wichtig für die Diagramme im Pop-up)
...
if GOOGLE_API_KEY:
    ...
    # Aktualisiert auf das aktuelle Modell, um den 404-Fehler zu beheben
    model = genai.GenerativeModel('gemini-2.5-flash')
else:
    st.error("Bitte hinterlege den GEMINI_API_KEY in den Streamlit Cloud Secrets!")

# Session States initialisieren, damit Daten beim Öffnen des Pop-ups erhalten bleiben
...
def show_details_popup(ticker):
    ...
    with st.spinner(f"Lade tiefgehende Finanzdetails für {ticker}..."):
        try:
            ...
            # RECHNERISCHE AMPEL-LOGIK (RSI & MACD)
...
if st.button("🚀 Scan Starten"):
    st.session_state.scan_results = []  # Alten Zustand leeren
    ...
    if tickers:
        # Performance-Limitierung für API-Stabilität
        if len(tickers) > 50:
            ...
            with st.spinner("Verarbeite technische Signale und generiere KI-Analysen..."):
                for idx, ticker in enumerate(tickers):
                    ...
                    try:
                        ...
                        # Branchen-Filterung via Live-Info-Request
                        ...
                        # Wenn Kriterien erfüllt sind, Treffer in den Session State sichern
...
# ==============================================================================
# 6. ERGEBNIS-ANZEIGE & DETACHED POP-UP LOGIK
# ==============================================================================
    ...
    if st.session_state.scan_results:
        ...
        for idx, item in enumerate(st.session_state.scan_results):
            with st.container():
                ...
                # Der Klick triggert nun sauber das Overlay-Pop-up, ohne den State zu manipulieren
                if st.button(f"🔍 Vollständige Finanzdetails für {item['ticker']} anzeigen", key=f"btn_{item['ticker']}_{idx}"):
